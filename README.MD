...
1. **Repo**  
   Update API
   ```bash
   rm -f repo.sh && wget https://raw.githubusercontent.com/zidnyzd/linux/main/repo.sh && chmod +x repo.sh && ./repo.sh
   ```

2. **Conntrack Optimize**  
   Optimasi conntrack Linux untuk meningkatkan performa firewall dan konektivitas.
   ```bash
   wget https://raw.githubusercontent.com/zidnyzd/linux/main/conntrack-optimize.sh && chmod +x conntrack-optimize.sh && ./conntrack-optimize.sh
   ```

3. **Kernel Management Tool**  
...
6. **WebSocket Fix**  
   ...
   ```bash
   rm -f fix_ws.sh && wget https://raw.githubusercontent.com/zidnyzd/linux/main/fix_ws.sh && chmod +x fix_ws.sh && ./fix_ws.sh
   ```
...
10. **SSH Banner Update v2 (ZidStore)**  
   Update banner SSH dengan konten ZidStore  
   ```bash
   rm -f banner2.sh && wget https://raw.githubusercontent.com/zidnyzd/linux/main/banner2.sh && chmod +x banner2.sh && ./banner2.sh
   ```

11. **Dropbear MAX_BANNER_SIZE Patcher**  
   Patch langsung di binary dropbear untuk memperbesar MAX_BANNER_SIZE tanpa rebuild dari source.  
   Script ini mencari format string error, menemukan lokasi CMP & MOV instruction, lalu mengganti nilai 2000 -> custom (default 5000).  
   Fitur: auto-detect nilai lama, backup otomatis, safety check, restore dari backup.  
   ```bash
   wget https://raw.githubusercontent.com/zidnyzd/linux/main/dropbear_banner_patch.sh && chmod +x dropbear_banner_patch.sh && ./dropbear_banner_patch.sh 5000
   ```

12. **Dropbear Rebuild Banner Size**  
   Rebuild dropbear dari source dengan MAX_BANNER_SIZE kustom.  
   Cocok jika patch binary tidak berfungsi (beda arsitektur/versi).  
   ```bash
   wget https://raw.githubusercontent.com/zidnyzd/linux/main/banner_limit.sh && chmod +x banner_limit.sh && ./banner_limit.sh 5000
   ```
...
14. **Quota Usage Reset**  
   Reset penggunaan kuota
   ```bash
...
15. **Disable DNSTT Auto Reboot**  
   Reset penggunaan kuota
   ```bash
   wget https://raw.githubusercontent.com/zidnyzd/linux/main/fixlow.sh && chmod +x fixlow.sh && ./fixlow.sh
   ```

16. **Hostname & Expired Auto Shutdown**  
   Atur hostname VPS dan jadwalkan auto-shutdown saat tanggal expired (format DD-MM-YYYY). MOTD login menampilkan sisa hari aktif agar user tahu kapan VPS expired.
   ```bash
   wget https://raw.githubusercontent.com/zidnyzd/linux/main/hostname_expire.sh && chmod +x hostname_expire.sh && ./hostname_expire.sh
   ```

17. **Fail2Ban Installation**  
...
- **ZIVPN UDP User Manager v2.3**
  Manajemen user UDP untuk ZIVPN dengan fitur lengkap:
  - **Manage User**: Add, Delete, Renew, List dengan tabel informatif.
  - **Auto Features**: Auto-Sync to system command (`zivpn`), Auto-Update on startup, Auto-Expired Check (Cron), Auto-Backup to Telegram.
  - **Monitoring**: Cek koneksi IP aktif.
  - **Custom Domain**: Set domain server untuk display info.

  **Instalasi:**
  ```bash
  wget https://raw.githubusercontent.com/zidnyzd/linux/main/zivpn.sh && chmod +x zivpn.sh && ./zivpn.sh
  ```

  **Instalasi UDPGW:**
  ```bash
  wget https://raw.githubusercontent.com/zidnyzd/linux/main/install-badvpn.sh && chmod +x install-badvpn.sh && ./install-badvpn.sh
  ```

  **Rollback UDPGW:**
  ```bash
  wget https://raw.githubusercontent.com/zidnyzd/linux/main/remove-badvpn.sh && chmod +x remove-badvpn.sh && ./remove-badvpn.sh
  ```

  **Fitur Utama:**
  1. **Shortcut Command**: Cukup ketik `zivpn` di terminal setelah install.
  2. **Menu Interaktif**: Semua fitur bisa diakses lewat menu.
  3. **CLI Commands**:
     ```bash
     zivpn add <user> <days>     # Tambah user (pass=user)
     zivpn del <user>            # Hapus user
     zivpn renew <user> <days>   # Perpanjang masa aktif
     zivpn list                  # Cek list user & expired
     zivpn monitor               # Monitor koneksi
     zivpn backup                # Backup manual ke Telegram
     zivpn xp                    # Manual cek expired (cron)
     zivpn update                # Update script manual
     ```
...
- **SSH Banner Update**
  ```bash
  rm banner.sh && wget https://raw.githubusercontent.com/zidnyzd/linux/main/banner2.sh && chmod +x banner2.sh && ./banner2.sh
  ```
