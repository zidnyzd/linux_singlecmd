...
change_password() {
    ...
    while IFS=':' read -r l_user l_pass l_exp; do
        if [[ "$l_user" == "$user" ]]; then
            echo "$l_user:$new_pass:$l_exp" >> "$temp_db"
        else
            echo "$l_user:$l_pass:$l_exp" >> "$temp_db"
        fi
    done < "$USER_DB"
    ...
    clear
    echo -e "${BLUE}=========================================${NC}"
    echo -e "          PASSWORD UPDATED               "
    echo -e "${BLUE}=========================================${NC}"
    echo -e " Username   : ${GREEN}$user${NC}"
    echo -e " New Pass   : ${GREEN}$new_pass${NC}"
    echo -e "${BLUE}=========================================${NC}"
    echo -e " ${YELLOW}Gunakan password baru di client ZIVPN${NC}"
    echo -e "${BLUE}=========================================${NC}"
    read -p "Press Enter to return..."
}
...
monitor_top_ip() {
    echo -e "${YELLOW}Top IP by connections/traffic (UDP 5667 and 6000-19999)...${NC}"
    
    # Pastikan conntrack tersedia
    if ! command -v conntrack &> /dev/null; then
        apt-get install -y conntrack > /dev/null 2>&1
    fi

    # Kumpulkan statistik: jumlah koneksi & bytes per sumber IP
    local table=$(conntrack -L -p udp 2>/dev/null | awk '
        /^udp/ {
            src=""; dport=""; bytes=0;
            for(i=1;i<=NF;i++){
                if($i ~ /^src=/){split($i,a,"="); src=a[2]}
                if($i ~ /^dport=/){split($i,a,"="); dport=a[2]}
                if($i ~ /^bytes=/){split($i,a,"="); bytes=a[2]}
            }
            # Hitung baik port backend 5667 maupun range forwarding 6000-19999
            if(src!="" && (dport=="5667" || (dport+0>=6000 && dport+0<=19999))){
                c[src]++; b[src]+=bytes;
            }
        }
        END{
            ...
            printf "%-20s %-10s %-12s\n","IP","Conns","MB";
            for(ip in b){
                printf "%-20s %-10d %-12.2f\n", ip, c[ip], b[ip]/1024/1024;
            }
        }')

    if [[ -z "$table" ]]; then
        echo "No active conntrack entries for UDP 5667."
    else
        echo "$table" | sort -k3 -nr | head
    fi

    echo "-------------------------------------------------------------"
    echo -e "${YELLOW}Tip:${NC} IP tertinggi bisa diblokir sementara via firewall."
    read -p "Press Enter to return..."
}
...
check_expired_cron() {
    ...
    # Loop manual agar tidak tergantung quoting awk yang rumit
    local l_user l_pass l_exp
    while IFS=':' read -r l_user l_pass l_exp; do
        if [[ -z "$l_user" ]]; then
            continue
        ...
        # Normalisasi expiry; jika bukan angka, anggap expired
        if [[ "$l_exp" =~ ^[0-9]+$ && "$l_exp" -gt "$now" ]]; then
            echo "$l_user:$l_pass:$l_exp" >> "$tmp_db"
        else
            echo "$l_user" >> "$removed_file"
        fi
    done < "$USER_DB"

    # Cek apakah ada perubahan jumlah baris (artinya ada user expired yg dihapus)
    local old_lines=$(wc -l < "$USER_DB")
    local new_lines=$(wc -l < "$tmp_db" 2>/dev/null)
    if [[ -z "$new_lines" ]]; then new_lines=0; fi
    ...
    if [ "$old_lines" -ne "$new_lines" ]; then
        ...
        # Notifikasi Telegram jika konfigurasi tersedia & ada yang terhapus
        if [ -f "$removed_file" ] && [ -s "$removed_file" ] && [ -f "/etc/zivpn/telegram.conf" ]; then
            source "/etc/zivpn/telegram.conf"
            if [[ -n "$ZIVPN_TG_TOKEN" && -n "$ZIVPN_TG_CHATID" ]]; then
                local count_removed=$(wc -l < "$removed_file")
                local removed_list=$(paste -sd"%0A- " "$removed_file")
                local msg="ZIVPN: ${count_removed} user expired dihapus:%0A- ${removed_list}"
                curl -s -X POST \
                    "https://api.telegram.org/bot${ZIVPN_TG_TOKEN}/sendMessage" \
                    -d chat_id="$ZIVPN_TG_CHATID" \
                    -d text="$msg" \
...
menu() {
    ...
    echo -e "6.  List Users (Check)"
    echo -e "7.  Monitor Connections"
    echo -e "8.  Top IP (Traffic)"
    echo -e "9.  Backup to Telegram"
    echo -e "10. Restore Data"
    echo -e "11. Install / Re-Install ZIVPN"
    echo -e "12. Uninstall ZIVPN"
    echo -e "13. Update Script"
    echo -e "14. Set Auto-Backup Time"
    echo -e "15. Set/Change Domain"
    echo -e "16. Install API Service"
    echo -e "17. Exit"
    echo -e "${BLUE}=========================================${NC}"
